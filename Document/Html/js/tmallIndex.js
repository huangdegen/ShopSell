...
function OrderResume() {
	...
	document.getElementById("pxfs").value = "";
	document.getElementById("sjpx").value = "";
	document.getElementById("ishyi").value = ""; //70906;
	hrefgo('1');
}
...
function shyiClk() {
	/*var sval = $('#ishyi').val();
	if(sval!=null && sval=="1") {
		document.getElementById("ishyi").value = "";
	}else {
		document.getElementById("ishyi").value = "1";
	}*///70906;
	document.getElementById("ishyi").value = "1";
	hrefgo('1');
}
...
function hrefgo(pcflag)
{
	...
	//双十一,60928:
	var shyi = trim(document.getElementById("ishyi").value);
	hrefurl += "-"+shyi;
	...
	if(sid != "") {
		try{
			...
			spIndexDwr.SaveSearchRec("5","2",sid,
			{
	      ...
	      callback:function(datalist) {
			...
			if(datalist != null && datalist.length > 0) {
				...
				for(var i=0;i<datalist.length;i++) {
					...
					if((datalist[i].shoptag) != null && (datalist[i].shoptag).indexOf('sbq1') >= 0) {
						dhstr = "是";
					}else {
						dhstr = "否";
					}
					...
					str += "    <h2 class=\"linkTitle\"><a href=\"shopsGet-"+datalist[i].id+".html\" title=\""+datalist[i].spTitle+"\" target=\"_blank\">"+datalist[i].spTitle+"</a></h2>";
					str += "    <ul><li><span>所属行业</span>"+datalist[i].typeName+"</li><li><span>消 保 金</span>"+szStr(datalist[i].xbMoney,"元","-")+"</li></ul>";
...
//					str += "    <ul><li><span>老客户数</span>"+szStr(datalist[i].kehu,"","-")+"</li><li><span>违规扣分</span><b>"+ziStr(datalist[i].koufenA,"")+" - "+ziStr(datalist[i].koufenB,"")+" - "+ziStr(datalist[i].koufenC,"")+"</b></li></ul>";
					...
					str += "    <ul><li><span>所在地区</span>"+datalist[i].sarea+"</li><li><span>技术年费</span>"+szStr(datalist[i].yearPrice,"元","-")+"</li></ul>";
					str += "    <ul><li><span>商城类型</span>"+sctype+"</li><li><span>动态评分</span><div class=\"dtpfxht\"><i class=\""+ziStr(datalist[i].msBi,"default")+"\"></i><i class=\""+ziStr(datalist[i].fwBi,"default")+"\"></i><i class=\""+ziStr(datalist[i].fhBi,"default")+"\"></i></div></li></ul>";
					str += "    <ul><li><span>商标类型</span>"+sbtype+"</li><li><span>违规扣分</span><b>"+ziStr(datalist[i].koufenA,"")+" - "+ziStr(datalist[i].koufenB,"")+" - "+ziStr(datalist[i].koufenC,"")+"</b></li></ul>";
					str += "    <ul><li><span>纳税资质</span>"+ziStr(datalist[i].nzzType,"-")+"</li><li><span>是否带货</span>"+dhstr+"</li></ul>";
